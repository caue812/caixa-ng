...
export class ClienteComponent {
  ...
  // evento que é executado quando o componente 
  ngOInit(){
  ...
  private cadastrar() {
    ...
    // Adicionado este objeto na lista de clientes
  ...
  salvarEmLocalStorage(){
    ...
    const clienteString = JSON.stringify(this.clientes);
    // Armazenando utilizando a chave 'cliente' a string com a lista de clientes
    localStorage.setItem("clientes", clienteString); 
  }

  carregarClientesDoLocalStorage(){
    // Obter doo localStorage(armaxenamento no navegador) utilizando a chave clientes
    ...
    // Percorre cada um dos clientes para utilizar o idAtual com o maior id dos clientes cadastrados
  ...
  apagar(cliente: Cliente){
    const confirmacao = confirm(`Deseja realmente apagar o cliente'$(cliente.nome)'?`);
    if (confirmacao !== true)
      return;
  ...
  preencherCampoParaEditar(cliente: Cliente){
